...
| Feature | Artifact | Next action | Last reviewed |
| ------- | -------- | ----------- | ------------- |
| Dual-engine bind-mount ownership | `dual-engine-bind-mount-ownership/NOTES.md` | Host proofs done 2026-09-15 on universe_media_api after fixing the per-entry fork walks (see issue 01/02 Comments): uid_map, inner chown, `0:0 → U:U` and old-map startup repair, 17 ms scan, doctor remap. Remaining before archive: the postgres cross-engine round trip (issue 01, last criterion) — run `docker compose up` for the db services inside the Container and confirm no restart loop. | 2026-09-15 |
